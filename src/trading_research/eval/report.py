...
# ---------------------------------------------------------------------------
# Multi-run composite ranking report (session 27)
# ---------------------------------------------------------------------------
...
def generate_multi_run_report(
    ...
    """Generate a standalone HTML report ranking multiple backtest runs by composite score.

    Reads summary.json from each run directory, computes composite scores, and
    renders a self-contained dark-themed HTML page with:
    - Top X strategies by composite score (with formula explanation)
    - Full run metadata table for all runs

    This is the multi-strategy entry point added in session 27.  It does not
    modify the per-run report generated by generate_report() — it produces a
    separate comparison page.

    Parameters
    ----------
    run_dirs : list[Path]
        List of backtest run directories, each containing summary.json.
        Directories without summary.json are silently skipped.
    output_path : Path
        Where to write the HTML file (e.g. ``runs/comparison.html``).
    x : int
        Number of top strategies to feature in the ranking table.
    min_trades : int
        Minimum trade count for inclusion in the ranking.
    title : str
    ...
    """
    from trading_research.eval.ranking import render_composite_ranking_html
    ...
    for run_dir in run_dirs:
        ...
        # Tag with run identity for display.
        strategy_id = (
            summary.get("strategy_id")
            or run_dir.parent.name  # e.g. "zn-macd-pullback-v1"
        )
        run_ts = run_dir.name
        summary["strategy_id"] = f"{strategy_id} / {run_ts}"
        summaries.append(summary)

    ranking_html = render_composite_ranking_html(summaries, x=x, min_trades=min_trades)

    # Build a full table of all runs for reference.
    all_rows = []
    def _fmt(val: object, spec: str, suffix: str = "") -> str:
        """Format a value with spec if numeric, else 'N/A'."""
        if val is None:
            return "N/A"
        try:
    ...
    for s in summaries:
        ...
        all_rows.append(
            f"<tr>"
            f"<td style='padding:0.4em 0.7em;color:#93c5fd;text-align:left;'>{s.get('strategy_id','')}</td>"
            f"<td style='padding:0.4em 0.7em;text-align:right;color:#e2e8f0;'>{_fmt(pf, '.3f')}</td>"
            f"<td style='padding:0.4em 0.7em;text-align:right;color:#e2e8f0;'>{_fmt(dd, '.1f', '%')}</td>"
            f"<td style='padding:0.4em 0.7em;text-align:right;color:#e2e8f0;'>{int(n):,}</td>"
            f"<td style='padding:0.4em 0.7em;text-align:right;color:#e2e8f0;'>{_fmt(sharpe, '.2f')}</td>"
            f"<td style='padding:0.4em 0.7em;text-align:right;color:#e2e8f0;'>{_fmt(calmar, '.3f')}</td>"
            f"</tr>"
        )

    th = "background:#334155;color:#e2e8f0;padding:0.5em 0.7em;"
    all_table = (
        f"<h2 style='color:#e2e8f0;'>All Runs ({len(summaries)})</h2>"
        f"<table style='width:100%;border-collapse:collapse;font-size:0.88em;'>"
        f"<thead><tr>"
        f"<th style='{th}text-align:left;'>Run</th>"
        f"<th style='{th}text-align:right;'>Profit Factor</th>"
        f"<th style='{th}text-align:right;'>Max DD</th>"
        f"<th style='{th}text-align:right;'>Trades</th>"
        f"<th style='{th}text-align:right;'>Sharpe</th>"
        f"<th style='{th}text-align:right;'>Calmar</th>"
        f"</tr></thead>"
        f"<tbody>{''.join(all_rows)}</tbody>"
        f"</table>"
    ) if all_rows else "<p style='color:#94a3b8;'>No valid runs found.</p>"
...
<style>
  ...
  h1   {{ color: #f1f5f9; border-bottom: 1px solid #334155; padding-bottom: 0.4em; }}
  table tr:nth-child(even) {{ background: #1e293b; }}
  table tr:hover {{ background: #263248; }}
</style>
</head>
<body>
<h1>{title}</h1>
