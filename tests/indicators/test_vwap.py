...
class TestSessionVWAP:
    def test_resets_on_gap(self):
        ...
        vwap = compute_session_vwap(df)['vwap_session']
    ...
    def test_no_lookahead(self):
        ...
        df = make_ohlcv(100)
        full = compute_session_vwap(df)['vwap_session']
        partial = compute_session_vwap(df.iloc[:51])['vwap_session']
...
class TestWeeklyVWAP:
    ...
    def test_resets_at_new_week(self):
        ...
        vwap = compute_weekly_vwap(df)['vwap_weekly']
    ...
    def test_no_lookahead(self):
        df = self._multi_week_df()
        full = compute_weekly_vwap(df)['vwap_weekly']
        partial = compute_weekly_vwap(df.iloc[:11])['vwap_weekly']
...
class TestMonthlyVWAP:
    ...
    def test_resets_at_new_month(self):
        ...
        vwap = compute_monthly_vwap(df)['vwap_monthly']
        # First bar of Feb (index 10) should reset.
        assert vwap.iloc[10] == pytest.approx(df["close"].iloc[10], rel=1e-6)

    def test_no_lookahead(self):
        df = self._multi_month_df()
        full = compute_monthly_vwap(df)['vwap_monthly']
        partial = compute_monthly_vwap(df.iloc[:11])['vwap_monthly']
        assert full.iloc[10] == pytest.approx(partial.iloc[-1], rel=1e-6)
