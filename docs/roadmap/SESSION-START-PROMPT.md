...
You are starting session **{{SESSION_NUMBER}}** of the Trading Research platform. This is a personal quant trading research desk. Code goes into `src/trading_research/`, configs into `configs/`, tests into `tests/`.

**Before writing any code, read the following files in this exact order:**

1. `docs/roadmap/session-specs/session-{{SESSION_NUMBER}}.md` — the specification for this session. This is your contract.
2. `CLAUDE.md` — project conventions, standing rules, voice and posture.
3. `.claude/rules/quant-mentor.md` — the quant mentor persona. Speaks up on market logic.
4. `.claude/rules/data-scientist.md` — the data scientist persona. Speaks up on statistical integrity.
5. `.claude/rules/platform-architect.md` — the platform architect persona. Speaks up on system design.
6. `docs/roadmap/sessions-23-50.md` — the overall roadmap, so you understand where this session fits.
